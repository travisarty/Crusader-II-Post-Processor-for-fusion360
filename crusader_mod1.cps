...
var validateLengthCompensation = getSetting("outputToolLengthCompensation", false); // disable validation when outputToolLengthCompensation is disabled
...
function onCircular(clockwise, cx, cy, cz, x, y, z, feed) {
  ...
  if (isFullCircle()) {
    ...
    switch (getCircularPlane()) {
    case PLANE_XY:
      //writeBlock(gMotionModal.format(clockwise ? 2 : 3), xOutputcirc.format(x), yOutputcirc.format(y), iOutput.format(cx - start.x), jOutput.format(cy - start.y), getFeed(feed));
      writeBlock(gMotionModal.format(clockwise ? 2 : 3), xOutputcirc.format(x), yOutputcirc.format(y), iOutput.format(cx), jOutput.format(cy), getFeed(feed));
      break;
    case PLANE_ZX:
      //writeBlock(gMotionModal.format(clockwise ? 2 : 3), xOutputcirc.format(x), zOutputcirc.format(z), iOutput.format(cx - start.x), kOutput.format(cz - start.z), getFeed(feed));
      writeBlock(gMotionModal.format(clockwise ? 2 : 3), xOutputcirc.format(x), zOutputcirc.format(z), iOutput.format(cx - start.x), kOutput.format(cz - start.z), getFeed(feed));
      break;
    case PLANE_YZ:
      //writeBlock(gMotionModal.format(clockwise ? 2 : 3), yOutputcirc.format(y), zOutputcirc.format(z), jOutput.format(cy - start.y), kOutput.format(cz - start.z), getFeed(feed));
      writeBlock(gMotionModal.format(clockwise ? 2 : 3), yOutputcirc.format(y), zOutputcirc.format(z), jOutput.format(cy - start.y), kOutput.format(cz - start.z), getFeed(feed));
      break;
    default:
    ...
    switch (getCircularPlane()) {
    case PLANE_XY:
      writeBlock(gMotionModal.format(clockwise ? 2 : 3), xOutputcirc.format(x), yOutputcirc.format(y), zOutputcirc.format(z), "R" + rFormat.format(r), getFeed(feed));
      break;
    case PLANE_ZX:
      writeBlock(gMotionModal.format(clockwise ? 2 : 3), xOutputcirc.format(x), yOutputcirc.format(y), zOutputcirc.format(z), "R" + rFormat.format(r), getFeed(feed));
      break;
...
function writeInitialPositioning(position, isRequired, codes1, codes2) {
  ...
  writeStartBlocks(isRequired, function() {
    ...
    if (currentSection.isMultiAxis() && getSetting("workPlaneMethod.prepositionWithTWP", true) && getSetting("workPlaneMethod.useTiltedWorkplane", false) &&
      ...
      //writeBlock(gOffset, additionalCodes[1]); // omit Z-axis output is desired
